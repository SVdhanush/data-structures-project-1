     	...
     	for(i=0;i<10;i++)
     	{
     		...
     		for(j=0;j<data1[i].no_of_days&&flag==1;j++)
     		{
     			if(strcmp(data1[i].stock[j].Present_date,Required_Date)==0)
     			  ...
     			  temp[i].stock[0]=data1[i].stock[j];
     			  flag=0;
     			 // k++;
				   }
			 }
...
void percentage_gain(company data[],char s_date[],char e_date[])	
{
    ...
    if(Per_gain>max)
    {
    	...
    	max=Per_gain;
         ...
         printf("the stock with maximum gain is %s and the maximum gain is   %d \n ",temp,max);
...
void Percentage_gain(company data[],char s_date[],char e_date[])	
{
    ...
    if(Per_gain<min)
    {
    	...
    	min=Per_gain;
         ...
         printf("the stock with minimum gain is %s and the minimum gain is   %d  \n ",temp,min);
